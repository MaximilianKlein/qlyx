...
const ButtonForm = ({ question }: { question: Question }) => (
  <form hx-post="/backend/adm" hx-trigger="submit">
    <div class="max-w-2xl mx-auto bg-white p-6 rounded shadow-md flex justify-between items-center">
      <span>
        {question.question}
        {question.startTime !== null ? (
          <div class="font-bold color-red">
            ACTIVE ({formatDate(question.startTime)})
          </div>
        ) : (
          ""
        )}
      ...
      />
      <button
        style="background-color: #3b82f6; color: #ffffff; font-weight: bold; padding: 8px 16px; border-radius: 9999px; cursor: pointer; border: none; outline: none; transition: background-color 300ms ease-in-out;"
        onmouseover="this.style.backgroundColor='#2b6cb0';"
        onmouseout="this.style.backgroundColor='#3b82f6';"
        type="submit"
        class="bg-blue-500 text-white font-bold py-2 px-4 rounded-full cursor-pointer transition duration-300 ease-in-out hover:bg-blue-700"
      >
        Activate!
      </button>
    </div>
...
export default (app: any) =>
  app
    .get("/", async ({ cookie, set }: any) => {
      if (cookie.admpwd !== adminPassword()) {
        set.status = 401;
      ...
      const questions = await db
        .select()
        .from(question)
        .orderBy(asc(question.questionId));
